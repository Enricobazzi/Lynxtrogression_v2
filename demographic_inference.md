...
Now I can optimize each bootstrap generated by [make_block_bootstrap.py](src/demographic_inference/make_block_bootstrap.py) using the dadi function [optimize_log](https://dadi.readthedocs.io/en/latest/api/dadi/Inference.html#dadi.Inference.optimize_log)

For this optimization I prepared two python scripts:
- [gadma2_best_dadi_models.py](src/demographic_inference/gadma2_best_dadi_models.py): this contains a model function for each of the models `model_func_<model>`, as well as a function to extract their best scoring parameters from the gadma2 run `p0_<model>` and their names `pnames_<model>`
- [dadi_optimization.py](src/demographic_inference/dadi_optimization.py): this script will run the optimizations for a specific dataset (either real data with `--vcf` and `--popinfo` or bootstrap data with `--boot`), model (`--model`), population pair (`--pop_pair`) and write the results to a table (`--csv`)

The following hyperparameters for the optimization are defined in the `optimization_hyperparams` function of the optimization script:
```
...
And the following bounds for parameter values:
```
fixed_p = [p if p == 0.0 else None for p in params]
lower_bound = [p * 0.1 for p in params]
upper_bound = [p * 10 for p in params]
```

I run [dadi_optimization.py](src/demographic_inference/dadi_optimization.py) for each of the 100 bootstrap replicates and the real data in a separate slurm job:
```
# REAL DATA
vcf=/mnt/netapp2/Store_csebdjgl/lynx_genome/lynx_data/mLynRuf2.2_ref_vcfs/lynxtrogression_v2.autosomic_scaffolds.filter4.lpa-wel_pair.miss_fil.rd_fil.variant.nogenes.vcf
popinfo=data/demographic_inference/lpa-wel.popmap.txt
pop_pair=lpa-wel

for model in 12_9 6_2 20_7; do
    csv=data/demographic_inference/${pop_pair}_CI/${pop_pair}_${model}/${pop_pair}.real_data.opti.csv
    sbatch \
        --job-name=${model}.real.opti \
        --output=logs/demographic_inference/${pop_pair}.${model}.real_data.optimization.out \
        --error=logs/demographic_inference/${pop_pair}.${model}.real_data.optimization.err \
        --time=00-02:00:00 \
        --ntasks=1 --cpus-per-task=1 --mem=8G \
        --wrap="module load cesga/system miniconda3/22.11.1-1 && \
            source activate /mnt/netapp1/Store_CSIC/home/csic/eye/eba/gadma2 && \
            python src/demographic_inference/dadi_optimization.py \
                --vcf ${vcf} --popinfo ${popinfo} --model ${model} --pop_pair ${pop_pair} --csv ${csv}"
done
...
for n in {0..99}; do
    boot=data/demographic_inference/${pop_pair}_bootstrap/${pop_pair}_${n}.fs
    csv=data/demographic_inference/${pop_pair}_CI/${pop_pair}_${model}/${pop_pair}.boot_${n}.opti.csv
    sbatch \
        --job-name=${model}.${n}.opti \
        --output=logs/demographic_inference/${pop_pair}.${model}.boot_${n}.optimization.out \
        --error=logs/demographic_inference/${pop_pair}.${model}.boot_${n}.optimization.err \
        --time=00-02:00:00 \
        --ntasks=1 --cpus-per-task=1 --mem=8G \
        --wrap="module load cesga/system miniconda3/22.11.1-1 && \
            source activate /mnt/netapp1/Store_CSIC/home/csic/eye/eba/gadma2 && \
            python src/demographic_inference/dadi_optimization.py \
                --boot ${boot} --model ${model} --pop_pair ${pop_pair} --csv ${csv}"
done
```

I then create a table with the optimization results for the bootstraps and real data with the following bash command:
```
pop_pair=lpa-wel

for model in 12_9 6_2 20_7; do
    header=$(head -1 data/demographic_inference/${pop_pair}_CI/${pop_pair}_${model}/${pop_pair}.real_data.opti.csv)
    echo "data,$header" > data/demographic_inference/${pop_pair}_CI/${pop_pair}_${model}.opti_table.csv
    for csv in $(ls data/demographic_inference/${pop_pair}_CI/${pop_pair}_${model}/*.csv); do
        name=$(basename $csv | cut -d'.' -f2)
        echo "$name,$(tail -1 $csv)" >> data/demographic_inference/${pop_pair}_CI/${pop_pair}_${model}.opti_table.csv
    done
done
```
...
### Running optimizations of best GADMA2 results OLD

From running GADMA2 I obtained 3 almost equally likely models which were models 12_9, 6_2 and 20_7.

To fully optimize the parameters for these models I will run some optimization rounds using dadi's function [optimize_log_fmin](https://dadi.readthedocs.io/en/latest/api/dadi/Inference.html#dadi.Inference.optimize_log_fmin).

For this optimization I prepared two python scripts:
- [gadma2_best_dadi_models.py](src/demographic_inference/gadma2_best_dadi_models.py): this contains a model function for each of the models `model_func_<model>`, as well as a function to extract their best scoring parameters from the gadma2 run `p0_<model>` and their names `pnames_<model>`
- [dadi_optimizaion.py](src/demographic_inference/dadi_optimization.py): this script will run the optimizations for a specific dataset (`--vcf` and `--popinfo`), model (`--model`), population pair (`--pop_pair`) and write the results to a table (`--csv`)
...
# The maximum number of iterations for the optimization
maxiter = 50
# The number of repetitions of the optimization command
repetitions = 50
# Folds perturbation of the initial parameters
fold = 1
```

For each repetition, the optimization will be run after perturbing the initial parameters using the [dadi.Misc.perturb_params](https://dadi.readthedocs.io/en/latest/api/dadi/Misc.html#dadi.Misc.perturb_params) as such:
```
params = [...]
fixed_p = [p if p == 0.0 else None for p in params]
lower_bound = [p * 0.1 for p in params]
upper_bound = [p * 10 for p in params]
pert_params = dadi.Misc.perturb_params(params, upper_bound = upper_bound, lower_bound = lower_bound, fold = fold)
```
using the `fold` hyperparameters defined above. If the log-likelihood of a repetition is better than the previous repetition, the initial parameters will be changed to the optimized ones, if not the new repetition will start with the same initial parameters.

I run 10 optimizations for each model:
```
vcf=/mnt/netapp2/Store_csebdjgl/lynx_genome/lynx_data/mLynRuf2.2_ref_vcfs/lynxtrogression_v2.autosomic_scaffolds.filter4.lpa-wel_pair.miss_fil.rd_fil.variant.nogenes.vcf
popinfo=data/demographic_inference/lpa-wel.popmap.txt
pop_pair=lpa-wel
...
for model in 12_9 6_2 20_7; do
    for run in {1..10}; do
        echo "sbatch optimization $run of $model"
        csv=data/demographic_inference/${pop_pair}_optimization/${pop_pair}.${model}.${run}.optimization.csv
    sbatch \
        --job-name=${model}.${run}.opti \
        --output=logs/demographic_inference/${pop_pair}.${model}.${run}.optimization.out \
        --error=logs/demographic_inference/${pop_pair}.${model}.${run}.optimization.err \
        --time=00-06:00:00 \
        --ntasks=1 --cpus-per-task=1 --mem=8G \
        --wrap="module load cesga/system miniconda3/22.11.1-1 && \
            source activate /mnt/netapp1/Store_CSIC/home/csic/eye/eba/gadma2 && \
...
for model in 12_9 6_2 20_7; do
    for run in {1..10}; do
        csv=data/demographic_inference/${pop_pair}_optimization/${pop_pair}.${model}.${run}.optimization.csv
        if [ $(wc -l < $csv) -eq 42 ]; then
            echo "$model $run complete!"
        else
            echo "sbatch optimization $run of $model : $(wc -l < $csv)"
        fi
    done
done
...
### Obtaining confidence intervals from best runs OLD
...
This generates a table called `result_table_converted.csv` in the same folder of the CI results which I use to calculate confidence intervals of physical units using the gadma script `gadma-get_confidence_intervals`:
```
pair=lpa-wel
for run in 12_9 6_2 20_7; do
    gadma-get_confidence_intervals \
        data/demographic_inference/${pair}_CI/${pair}_${run}/result_table_converted.csv \
        --log --acc 12 | cut -f1,2,3 | tr -d ':' \
        > data/demographic_inference/${pair}_CI/${pair}_${run}/confidence_intervals.tsv
done

cat data/demographic_inference/lpa-wel_CI/lpa-wel_6_2/confidence_intervals.tsv | awk '{print $1, "["int ($2) " - " int($3)"]"}'
```

This will calculate confidence intervals for each parameter from the distribution of log transformed values obtained by running local search 100 bootstrap replicates of the data. The limit values are calculated as `min = mean - 1.96 * sd` and `max = mean + 1.96 * sd`.
