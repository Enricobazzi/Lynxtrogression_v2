...
def optimization_hyperparams():
    ...
    fold = 1
    return pts, maxiter, repetitions, fold
...
def get_data(vcf_file:str, popinfo: str, pop_pair: str):
...
def main() -> None:
    ...
    with open(log_file, 'w') as f:
        f.write(f"pop_pair,model,repeat,log-likelihood,{pnames_str}\n")
    
    # read data from vcf
    print("Reading data...")
    data = get_data(args.vcf, args.popinfo, args.pop_pair)
    
    # get model function
    model_func = get_model(args.model)
    
    # get optimization hyperparameters
    pts, maxiter, repetitions, fold = optimization_hyperparams()
    print(f'Optimization hyperparameters: pts={pts}, maxiter={maxiter}, repetitions={repetitions}')
    ...
    with open(log_file, 'a') as f:
        f.write(f"{args.pop_pair},{args.model},0,{ll_model},{','.join(str(val) for val in p0)}\n")

    # run i=repetitions optimizations
    for i in range(repetitions):
        print(f'Optimization {i+1}/{repetitions}')
        ...
        p0p = dadi.Misc.perturb_params(p0, upper_bound=upper_bound, lower_bound=lower_bound, fold=fold)
        
        # optimize
        opt_params = dadi.Inference.optimize_log_fmin(
            p0 = p0p, data = data,
        ...
        print(f'optimization {i+1} log-likelihood: {new_ll_model}')
        print(f'optimization {i+1} parameters: {opt_params}')
        
        # write results to log file
        with open(log_file, 'a') as f:
            f.write(f"{args.pop_pair},{args.model},{i+1},{new_ll_model},{','.join(str(val) for val in opt_params.tolist())}\n")
        
        # update parameters if new log-likelihood is better
        if new_ll_model > ll_model:
            print(f'{new_ll_model} > {ll_model}')
            print('new parameters accepted - next repetition starts with new parameters')
            ll_model = new_ll_model
            p0 = opt_params
        else:
            print(f'{new_ll_model} <= {ll_model}')
            print('new parameters rejected - next repetition starts with previous parameters')
