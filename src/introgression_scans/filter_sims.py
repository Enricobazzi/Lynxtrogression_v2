...
for s, block, anc_block in zip(segsites, ms_blocks, anc_blocks):
    if s >= n_sites:
        ws, we = get_ws_we(anc_block, n_sites, migration, pop_sizes)
        print(ws, we)
        if ws:
            f_block = filter_ms_block(block, ws, we)
            f_anc_block = filter_anc_block(anc_block, ws, we)
            ms_blocks_filtered.append(f_block)
            anc_blocks_filtered.append(f_anc_block)
