...
def remove_ancestral_populations(graph):
    ...
    for epoch in graph2.resolve().demes[1].epochs:
        graph2.add_migration(
            source=graph2.resolve().demes[0].name,
            dest=graph2.resolve().demes[1].name,
            start_time=epoch.start_time,
            end_time=epoch.end_time,
            rate=0.01
        )
        graph2.add_migration(
            source=graph2.resolve().demes[1].name,
            dest=graph2.resolve().demes[0].name,
            start_time=epoch.start_time,
            end_time=epoch.end_time,
            rate=0.01
        )
...
def get_ms_string(mscommand, path_to_msmodified, size1, size2, nreps, L):
    ...
    for command in commands:
        ...
        if flag not in ['-t', '-r', '-I', '-n', '-g', '-m', '-en', '-eg', '-ej', '-es', '-em']:
...
def get_tbs_list(mscommand):
    ...
    for command in commands:
        flag = command.split(' ')[0]
        if flag not in ['-t', '-r', '-I', '-n', '-g', '-en', '-eg', '-ej', '-es', '-m', '-em']:
            raise ValueError(f'Command from demography not contemplated in the script: {flag}')
        if flag == '-t':
            ...
            tbs_list.append(command.split(' ')[1])
            tbs_list.append(command.split(' ')[4])
    return tbs_list
...
def modify_graph_from_confint(graph, confint):
    ...
    # migration rates - order of 12 and 21 depends on how the demes graph migrations were added in remove_ancestral_populations()
    # I added them in the order 21, 12 (see source and dest in demes.Builder.add_migration)
    graph.migrations[0].start_time = params["t2"] + params["t3"] + params["t4"]
    graph.migrations[0].end_time = params["t3"] + params["t4"]
    graph.migrations[0].rate = params["m2_21"]
    graph.migrations[1].start_time = params["t2"] + params["t3"] + params["t4"]
    graph.migrations[1].end_time = params["t3"] + params["t4"]
    graph.migrations[1].rate = params["m2_12"]
    ...
    graph.migrations[3].start_time = params["t3"] + params["t4"]
    graph.migrations[3].end_time = params["t4"]
    graph.migrations[3].rate = params["m3_12"]
    graph.migrations[4].start_time = params["t4"]
    graph.migrations[4].end_time = 0
    graph.migrations[4].rate = params["m4_21"]
    graph.migrations[5].start_time = params["t4"]
    graph.migrations[5].end_time = 0
    graph.migrations[5].rate = params["m4_12"]
...
def sample_mu(mu, alpha=10) -> float:
    """
    Sample a mutation rate from a gamma distribution centered around mu.
    Uses a gamma distribution with shape parameter alpha=10 and scale=1/alpha
    to generate samples centered around the input mutation rate.
    """
    alpha = 10
    factor = np.random.gamma(alpha, scale=1/alpha)
    sampled_mu = mu * factor
    return sampled_mu

def sample_rec(rec, alpha=10) -> float:
    """
    Sample a recombination rate from a gamma distribution centered around rec.
    Uses a gamma distribution with shape parameter alpha=10 and scale=1/alpha
    to generate samples centered around the input recombination rate.
    """
    alpha = 10
    factor = np.random.gamma(alpha, scale=1/alpha)
    sampled_rec = rec * factor
    return sampled_rec
    ...
    with open('mig.tbs', 'w') as f:
        for _ in range(nreps):
            ...
            mscommand = get_ms_command(graph = graph, N0 = graph[pop1].epochs[0].end_size, migration = migration,
                                       L = L, samples = [get_sizes(pop1), get_sizes(pop2)], mu = sample_mu(mu), rec = sample_rec(rec), mtime = mtime)
